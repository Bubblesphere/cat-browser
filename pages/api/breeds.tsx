...
export const GetBreed = async (breedId: string) => {
  const breed: SearchImage[] = await searchImages.fetcher(
    searchImages.url('thumb', 1, 0, breedId)
  );

  return breed[0].breeds[0];
};

export const GetBreedWithImages = async (
  breedId: string,
  imageSize: ImageSize,
  limit: number,
  page: number
  ...
  const breed: SearchImage[] = await searchImages.fetcher(
    searchImages.url(imageSize, limit, page, breedId)
  );
