...
export default function useCatService() {
  ...
  const allBreeds = (limit: number, page: number) =>
    useSWR(
      allBreedsSchema.url(limit, page),
      allBreedsSchema.fetcher
    ) as responseInterface<Breed[], Error>;

  const getBreed = (breedId: string, limitImage: number, pageImage: number) =>
    useSWR(
      searchImageSchema.url('full', limitImage, pageImage, breedId),
      searchImageSchema.fetcher
    ) as responseInterface<Image[], Error>;
